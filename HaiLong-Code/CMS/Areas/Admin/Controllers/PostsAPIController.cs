...
namespace CMS.Areas.Admin.Controllers
{
    public class PostsAPIController : ApiController
    {
        ...
        public IQueryable<Post> GetPost(string att, string value)
        {
            ...
            //Phòng nghỉ trang chủ
            if (att == "phongNghi" && att != null && value != null)
            {
                int number = int.Parse(value);
                var model = db.Post.Where(p => p.idCategory == 2 && p.published == 1 && p.featured == 1).OrderByDescending(p => p.timeModified).Take(number);

                return model;
            }

            //nhà hàng & sự kiện
            if (att == "nhaHangSuKien" && att != null && value != null)
            {
                int number = int.Parse(value);
                var model = db.Post.Where(p => p.idCategory == 3 && p.published == 1 && p.featured == 1).OrderByDescending(p => p.timeModified).Take(number);

                return model;
            }

            //Dịch vụ khách sạn
            if (att == "dichVuKhachSan" && att != null && value != null)
            {
                int number = int.Parse(value);
                var model = db.Post.Where(p => p.idCategory == 4 && p.published == 1 && p.featured == 1).OrderByDescending(p => p.timeModified);
